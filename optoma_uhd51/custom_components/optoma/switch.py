...
class OptomaSwitch(SwitchEntity):
    async def async_update(self):
        self._is_on = await self.hass.async_add_executor_job(self._query_state)
    ...
    def _query_state(self):
        try:
            with telnetlib.Telnet(self._host, 23, timeout=1.0) as tn:
                ...
                response = tn.read_until(b"\r", timeout=1.0)
                _LOGGER.debug("Projector response: %s", response)
                return b"ON" in response.upper() or b"1" in response
